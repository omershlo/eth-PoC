function compareCurvePoints(p1, p2) {
  return (p1.x.toString('hex') === p2.x.toString('hex')) &&
    (p1.y.toString('hex') === p2.y.toString('hex'));
}
/*#######################################################################################*/
/*#######################################################################################*/
/*###################################### Server Sign #####################################*/
...
function serverSignStage1({serverCache,clientCache}) {
  ...
  const a1s_R1_concat = a1s.toString(Consts.HEX).concat(R1.x.toString('hex'));
  ...
  // c = H(t,G,R1) mod (q)
  const ts_g_R1_concat = ts.x.toString('hex').concat(utils.ec.g.x.toString('hex'), R1.x.toString('hex'));
 ...
 // const rs = v.minus(c.multiply(k1)); 
  ...
  const a2s_ts_rs_concat = a2s.toString(Consts.HEX).concat(ts.x.toString(Consts.HEX), rs.toString(Consts.HEX));
...
function serverSignStage2({ tc, rc, R2 ,serverCache,clientCache}) {
  ...
  // Computing c2
  serverCache['R2'] = R2;
  const tc_G_R2_concat = tc.x.toString('hex').concat(utils.ec.g.x.toString('hex'), R2.x.toString('hex'));
...
function serverSignStage3({c3,serverCache,clientCache,recid}){
  ...
  const signature_r = modulo(BigInteger(R.x.toString('hex'),Consts.HEX),Consts.q);
  ...
  const ver1 = verPoint.x.toString(16);
  const verR = signature_r.toString(16);
  console.log("a= " +verPoint.x.toString(16))
...
function clientSignStage2({ a1s, R1, a2s, ts, rs, com1s, com2s ,serverCache,clientCache}) {
  ...
  const a2s_ts_rs_concat = a2s.toString(Consts.HEX).concat(ts.x.toString('hex'), rs.toString(Consts.HEX));
  ...
  const tc_g_R1_concat = ts.x.toString('hex').concat(utils.ec.g.x.toString('hex'), R1.x.toString('hex'));
  ...
  const rc_x = modulo(BigInteger(R.x.toString('hex'),Consts.HEX),Consts.q);
  ...
  const RxBN = BigInteger(R.x.toString('hex'),16);
  const RyBN = BigInteger(R.y.toString('hex'),16);
